...
Runs the same three stages as the queue-driven consumer, synchronously, against
a local file. Useful for trying a document without Redis in the loop.

Usage:
    uv run python -m app.cli --receipt samples/receipt1.png --system oxn \
...
from app.core.config import DEFAULT_SYSTEM, VALID_SYSTEMS
...
from app.pipeline.orchestrator import stamp_records
from app.pipeline.write import write_assets, write_assets_to_db
...
def run(receipt: str, system: str, out: str) -> int:
    ...
    po_ref, header, items = extract_invoice(receipt)  # Stage 1 (items + header)
    if not items:
    ...
    stamp_records(records, items, header, proc, po_ref)
