"""Stage 2 — assign each extracted line item exactly one allowed category.

The agent (``app/agents/categorizer.py``) runs on the local model with a single
``list_categories()`` tool. This module owns everything around it: validating
what came back, forcing the category onto the allowed list, re-deriving the
parent group deterministically, and the fallback path.

The fallback matters. If the local model cannot drive tools, ``categorize``
transparently makes one structured call with the category list and the items in
a single prompt; if even that produces nothing usable, every item is kept under
a default category rather than dropped. Data is never lost to a model failure.
"""
...
from app.agents.categorizer import classify
from app.core.config import (
    DEFAULT_SYSTEM,
    category_names,
    group_for_category,
)
from app.core.json_utils import extract_json_list
from app.core.llm import get_openai_client
from app.core.logging_config import get_logger
from app.schemas.domain import AssetRecord, RawLineItem
from app.tools.categories import format_categories
...
def categorize(
    ...
    try:
        records = _finalize(classify(items, system), allowed, system)
        if records:
